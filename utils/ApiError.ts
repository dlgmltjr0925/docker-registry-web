...
export enum Status {
  ...
  /**
   * 요청에 대해서 하나 이상의 응답이 가능합니다. 사용자 에이전트 또는 사용자는 그중에 하나를 반드시 선택해야 합니다.
  ...
  /**
   * 이 응답 코드는 요청한 리소스의 URI가 일시적으로 변경되었음을 의미합니다. 새롭게 변경된 URI는 나중에 만들어질 수 있습니다.
  ...
  SEE_OTHER = 303,
  /**
   * 이것은 캐시를 목적으로 사용됩니다. 이것은 클라이언트에게 응답이 수정되지 않았음을 알려주며,
   ...
   * 서버가 지원해야 하는 유일한 방법은 GET와 HEAD이다. 이 코드는 반환하면 안됩니다.
   */
  NOT_IMPLEMENTED = 501,
  /**
...
export default class ApiError extends Error {
  ...
  constructor(constructionArg: ConstructionArgs) {
    super(JSON.stringify(constructionArg));
  }
}
